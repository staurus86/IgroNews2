...
Pipeline flow: RSS/HTML sources -> Parser -> TF-IDF NLP -> Google Trends + Keys.so APIs -> Claude LLM analysis -> Google Sheets -> Editorial approval
...
- `apis/llm.py` — Claude API with 3 prompt scenarios: trend forecast, news merging, search query generation
...
- LLM model: `claude-sonnet-4-20250514` (configured in `config.py`)
